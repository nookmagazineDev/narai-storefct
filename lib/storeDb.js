...
import { fetchSheetRows, fetchSheetRowsCsv, cellStr, cellNum, cellYmd, cellDateTime, parseDateTimeString, SHEET_GID, SHEET_NAME } from './sheets.js';
...
//
// ⚠️ gviz คืนเฉพาะแถวที่ผ่าน "ตัวกรอง" ที่เปิดค้างไว้ในชีท ถ้าจำนวนแถวที่ได้ดูน้อยผิดปกติ
// ให้สั่ง { csv: true, gid } เพื่ออ่านผ่าน export CSV ซึ่งไม่สนใจตัวกรอง — ดูเหตุผลเต็มที่
// fetchSheetRowsCsv() ใน lib/sheets.js
// ---------------------------------------------------------------------------

// คำที่ใช้ดูว่าแถวแรกของ CSV เป็นหัวตารางหรือเป็นข้อมูลจริง (ไม่ตัดแถวแรกทิ้งดื้อๆ)
const HEADER_HINTS = {
...
/**
 * อ่านแถวของแท็บหนึ่ง เลือกได้ว่าจะผ่าน gviz (ค่าเริ่มต้น) หรือ export CSV
 *
 * CSV ต้องรู้ gid เสมอ — แท็บที่ Apps Script สร้างเองตอนใช้งาน (ดึงข้อมูลใบเบิก/ยกเลิกใบเบิก)
 * ไม่รู้ gid ล่วงหน้า ผู้เรียกจึงต้องส่ง gid มาเองถ้าจะใช้โหมด CSV กับแท็บพวกนั้น
 * (เปิดแท็บนั้นในเบราว์เซอร์แล้วดูเลขท้าย URL)
 */
async function loadRows(part, { gid, sheet }, opts = {}) {
  if (!opts.csv) {
    return fetchSheetRows(gid !== undefined ? { gid } : { sheet });
  }
  const csvGid = opts.gid !== undefined ? opts.gid : gid;
  if (csvGid === undefined) {
    throw new Error(`โหมด CSV ต้องระบุ gid ของแท็บ "${sheet}" ด้วย (เปิดแท็บนั้นแล้วดูเลขท้าย URL)`);
  }
...
export async function syncFulfillment(opts = {}) {
  const rows = await loadRows('fulfillment', { gid: SHEET_GID.fulfillment }, opts);
...
export async function syncReceiving(opts = {}) {
  const rows = await loadRows('receiving', { gid: SHEET_GID.receiving }, opts);
...
export async function syncFetched(opts = {}) {
  let rows;
  try {
    rows = await loadRows('fetched', { sheet: SHEET_NAME.fetched }, opts);
  } catch (err) {
    if (opts.csv) throw err; // โหมด CSV ที่พังคือตั้งค่าผิด ไม่ใช่ "ยังไม่มีแท็บ" — อย่ากลบเงียบ
...
export async function syncCancelled(opts = {}) {
  let rows;
  try {
    rows = await loadRows('cancelled', { sheet: SHEET_NAME.cancelled }, opts);
  } catch (err) {
    if (opts.csv) throw err; // โหมด CSV ที่พังคือตั้งค่าผิด ไม่ใช่ "ยังไม่มีแท็บ" — อย่ากลบเงียบ
    return { sheetRows: 0, upserted: 0, note: 'ยังไม่มีแท็บนี้ในชีท' };
  }
