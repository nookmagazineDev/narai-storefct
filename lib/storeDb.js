...
/**
 * ยอดคงเหลือรายไอเทมรวมทุกสาขา -> { [itemCode]: { remaining, name, unit } }
 *
 * มาแทนชีท "ยอดคงเหลือไอเทม" ซึ่งไม่เคยมีใครกรอกเลย (ว่างมาตลอด) คอลัมน์ "คงเหลือ" ในหน้า
 * สรุปส่งของจึงเป็นค่าว่างตลอดมา ทั้งที่ dbo.stock_count / stock_balance มีข้อมูลจริงอยู่แล้ว
 *
 * ใช้ getStockTotal ตัวเดียวกับหน้า "รวมสต๊อกทุกสาขา" — ยอดนับล่าสุดต่อ (สินค้า, สาขา) แล้ว
 * สาขาที่ยังไม่เคยนับถอยไปใช้ยอดยกมา กติกานั้นอยู่ฝั่ง SQL ที่เดียว สองหน้าจะได้ไม่คิดคนละแบบ
 */
export async function readItemBalance() {
  const rows = await callOffice('getStockTotal', {});
  const byCode = {};
  for (const r of rows || []) {
    const code = String(r.productId || '').trim();
    if (!code) continue;
    // totalRemaining เป็น '' เมื่อไม่มีสาขาไหนนับเลย — แปลงเป็น null ให้ตรงกับที่ตัวอ่านชีทเคยคืน
    const raw = r.totalRemaining;
    byCode[code] = {
      remaining: raw === '' || raw === null || raw === undefined ? null : Number(raw),
