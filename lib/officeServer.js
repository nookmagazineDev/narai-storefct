...
// ตรงกับค่าเริ่มต้นของ Narai-branch (lib/upstream.js) — เครื่อง NARAI-PIZZARIA ผ่าน Cloudflare Tunnel
// เดิมเป็น http://storenarai.dyndns.tv:8787 ซึ่ง README ของ office-server ระบุว่าเรียกจาก Vercel
// ไม่ถึงแล้ว (21/09/2026) ผลคือทุก action ของครัวกลางได้ "ตอบกลับมาไม่ใช่ JSON (HTTP 404)"
const DEFAULT_BASE = 'https://usage.khanoykorshabu.com';
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (res.status >= 500 && attempt < retries) {
        ...
        // office-server ตัวจริงตอบ JSON เสมอ แม้แต่คำสั่งที่ไม่รู้จัก (400) — 404 ที่ไม่ใช่ JSON แปลว่า
        // ไปถึงเว็บเซิร์ฟเวอร์ตัวอื่นที่ไม่มี /schedule คือที่อยู่ผิดเครื่อง บอกที่อยู่ไปด้วยจะได้แก้ถูกจุด
        if (!json && res.status === 404) {
          throw new DefiniteError(officeBase() === DEFAULT_BASE
            ? `${DEFAULT_BASE} ไม่มี /schedule — Cloudflare Tunnel อาจชี้ไปเครื่องที่ไม่ได้รัน office-server (ดู docs/troubleshooting-server.md ของ Narai-branch)`
            : `ที่อยู่เซิร์ฟเวอร์ที่ออฟฟิศไม่ถูกต้อง (${officeBase()} ไม่มี /schedule) — ` +
              `แก้ USAGE_API_BASE บน Vercel เป็น ${DEFAULT_BASE} หรือลบทิ้งให้ใช้ค่าเริ่มต้น`);
        }
